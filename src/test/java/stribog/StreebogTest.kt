...
internal class StreebogTest {
    ...
    @ParameterizedTest
    @MethodSource("args256")
    fun streebog256Test(input: ByteArray?, expected: ByteArray?) {
        assertArrayEquals(input, Streebog256().computeHash(expected))
    }

    @ParameterizedTest
    @MethodSource("args512")
    fun streebog512Test(input: ByteArray?, expected: ByteArray?) {
        assertArrayEquals(input, Streebog512().computeHash(expected))
    }
    ...
    companion object {
        ...
        private val testIn = "The quick brown fox jumps over the lazy dog".toByteArray(StandardCharsets.US_ASCII)
        ...
        @JvmStatic
        fun args256(): Stream<Arguments> = Stream.of(
            Arguments.of(
                testEmptyOut256, testEmptyIn
            ),
            Arguments.of(
                    testOut256, testIn
            ),
            Arguments.of(
                testRfc1Out256, testRfc1In
            ),
            Arguments.of(
                testRfc2Out256, testRfc2In
            ),
            Arguments.of(
                byteArrayOf(), null
            )
        ...
        @JvmStatic
        fun args512(): Stream<Arguments> = Stream.of(
            Arguments.of(
                testEmptyOut512, testEmptyIn
            ),
            Arguments.of(
                testOut512, testIn
            ),
            Arguments.of(
                testRfc1Out512, testRfc1In
            ),
            Arguments.of(
                testRfc2Out512, testRfc2In
            ),
            Arguments.of(
                byteArrayOf(), null
            )
